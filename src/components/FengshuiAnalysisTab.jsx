...
export default function FengshuiAnalysisTab() {
  ...
  const [systemPromptB, setSystemPromptB] = useState(
    '你是专业风水顾问。仅基于输入的真实元素与布局进行分析，若输入缺失或为null则不要臆造。严格输出JSON：{advice:[string], itemsToAdd:[{item,reason,area,direction,style,color,size,material}]}，不得输出其他文本。建议需与输入元素与方位直接相关。'
  )
  ...
  useEffect(() => {
    if (!promptBEdited) {
      ...
      setPromptB(`输入元素：\n${base}\n\n仅提取重要内容，按 JSON 模板输出风水建议与需添置物品清单。`)
    }
  ...
  const runStepB = async (elementsOverride = null) => {
    ...
    const timeoutMsB = 60000 // Doubao可能较慢，统一提升到60s
    ...
    const response = await fetch(`${API_ORIGIN || ''}/api/fengshui/advise`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
        ...
        // 仅当用户手动编辑过且非空时覆盖，否则走后端默认（包含 imageElements）
        prompt: (promptBEdited && String(promptB).trim().length > 0) ? promptB : undefined,
        provider: {
          ...provider,
          params: { ...provider.params }
        }
  ...
  const runStepC = async (adviceOverride = null) => {
    ...
    if (adviceJson) {
      if (Array.isArray(adviceJson.itemsToAdd)) {
        itemsToAdd = adviceJson.itemsToAdd
      } else if (Array.isArray(adviceJson['应添置物品清单'])) {
        itemsToAdd = adviceJson['应添置物品清单']
          .map((s) => {
            if (typeof s === 'string') {
              const item = s.replace(/（.*?）/g, '').trim()
              return { item }
            }
            return s
          })
